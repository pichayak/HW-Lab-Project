...
module character(
	...
	reg [9:0] x_corner = 320; //character start position - X-axis
	...
	(*ROM_STYLE = "block"*) reg [7:0] CHARACTER [0:747]; // size = 34*22 
...
// todo: correct value of WASD
    ...
    begin
    	...
    	if (button == 8'h77) // w - up
    		begin
    			y_corner <= y<=90 ? y_corner : y_corner - 8'd1; // <height
    		end
        else if (button == 8'h73) //s -down
    		begin
    			y_corner <= y>=290 ? y_corner : y_corner + 8'd1; // <height
    			//y_corner <= y_corner + 8'd1;
    		end
    	else if (button == 8'h61) //a left
    		begin
    			x_corner <= x<=220 ? x_corner : x_corner - 9'd1; // <width
    		end
    	else if (button == 8'h64) //d right
    		begin
    			x_corner <= x>=420 ? x_corner : x_corner + 9'd1; // <width
